...
#
# (not comprehensive)
# - escaping code is still broken, e.g. keymap \
# - statusbar start symbols aren't taken into consideration for calculations
# - mouse code really only works for a single setup
# }}}
#########
...
# }}}
# {{{ /set awl_tag <no|nochan|other|all> XXX NOT YET IMPLEMENTED
#     * shows the network tag in front of the window list items
#         no     : doesn't show a tag
#         nochan : shows tags in queries etc.
#         all    : shows tags in all items
# }}}
# {{{ /set awl_tag_display <string> XXX NOT YET IMPLEMENTED
#     * string : this string is used in place of $T in the awl_display_*
#                settings
# }}}
# {{{ /set awl_notag_display <string> XXX NOT YET IMPLEMENTED
#     * string : this string is used in place of $T in the awl_display_*
#                settings when no tag is to be shown
# }}}
# {{{ /set awl_lightbar_center <num> XXX NOT YET IMPLEMENTED
#     * num : the width that the lightbar can take. If you specify a width, the
#       currently active channel will be centred to this width.
#       specify -1 to use the maximum window width for centring or 0 to disable
#       centring.
# }}}
# {{{ /set awl_sbar_keymap_align <ON|OFF> XXX NOT YET IMPLEMENTED
#     * when enabled, makes each statusbar contain the windows that are mapped
#       to one 'row' on the keyboard, if you have set your keyboard layout in
#       that way.
# }}}
...
# }}}
# {{{ /set awl_automode <sbar|screen|fifo|emulate_lightbar> XXX
#     * this setting defines whether the window list is shown in statusbars or
#       whether the screen hack is used (from nicklist.pl)
#       Fifo mode is now recommended as it saves the flashing.
#       If you set adv_windowlist in lightbar emulation mode, it will not create
#       any statusbars and it will provide only a single lined item awl_0.
#       You can then make your own statusbar or add that item to any exisiting
#       statusbar in lightbar/chanact fashion. (XXX NOT YET IMPLEMENTED)
# }}}
# {{{ /set awl_keymap <string> XXX NOT YET IMPLEMENTED
#     * string : list of keys acceptable for shortcut keys
# }}}
# {{{ /set awl_keymap_automatic <sequential|from_itemname> <meta-> XXX NOT YET IMPLEMENTED
#     * sequential : assign keymaps from awl_keymap one after another to windows
#                    created
#       from_itemname : choose a keymap from awl_keymap but try to use the first
#                       available letter of itemname if possible
#       from_name : choose keymap from window name
#       disabled : don't automatically create key bindings
#       meta- : cascade keys with meta- once you run out of keys
# }}}
# {{{ /set awl_keymap_follow <ON|OFF> XXX NOT YET IMPLEMENTED
#     * if enabled, keymaps belong to the window and get changed when you move a
#       window
# }}}
# {{{ /set awl_keymap_store <name|setting|name+setting> XXX NOT YET IMPLEMENTED
#     * name : stores the keymap assigned with /awl key only in the window name.
#              you'll have to use /layout save to save your layout including
#              window names
#       setting : stores the keymap assigned with /awl key in the setting
#                 awl_keymap_store_setting in the form of "tag/itemname key"
#       name+setting : use both methods
# }}}
# {{{ /set awl_name_magic <char> XXX NOT YET IMPLEMENTED
#     * char : character that seperates keymap from name in /window name
# }}}
# {{{ /set awl_sort_windows [chan] [nochan] [nogroup|header] [virtual]  XXX NOT YET IMPLEMENTED
#     * chan    : sort channel windows
#       nochan  : sort queries etc.
#       nogroup : don't group windows by chatnet
#       header  : show chatnet headers
#       virtual : don't sort the windows, only sort them on display (see also
#                 awl_sort)
#     DEV HINT: order of items specifies which to sort first (chan nochan =
#     channels first, nochan chan = queries first). If in(ner)group is
#     specified, the sort behaviour goes above grouping, meaning: you have two
#     large blocks, chan and nochan, which can then be split into chatnets.
#     here's another try (ingroup nochan chan -header):
#     [[qnet[a,b,c],unet[jana,lena,linda]],[qnet[#perl],unet[#flirt]]]
#     without ingroup (nochan chan -header):
#     [qnet[a,b,c,#perl],unet[jana,lena,linda,#flirt]]
#     with nogroup (nochan chan nogroup):
#     [a,b,c,jana,lena,linda,#flirt,#perl]
#     reverse sorting (-chan -nochan):
#     [#flirt,linda,lena,jana,#perl,c,b,a]
#     -chan -nochan -noheader:
#     [#perl,c,b,a,#flirt,linda,lena,jana]
# }}}
# {{{ /set XXX NOT YET IMPLEMENTED XXX WANTED WANTED WANTED
#     Wanted: a setting to have different
#             statusbars for chans and nochans
#             
#     Wanted: a setting to show only chans, or only nochans
#             also a way to hide all windows that are named '..'
#             or have an item '..' or are of type '..'
#
#     Wanted: a setting to ignore hilights in certain windows
#
#     Wanted: a feature to show dcc gets &/| sends as awl items,
#             possibly with progress bar (%8 or custom style)
# }}}
# }}}
##########
# {{{ COMMANDS
...
#       fix it.
# }}}
# {{{ /awl key [[tag/]target|refnum] <key> XXX NOT YET IMPLEMENTED
#     * assigns the keybinding meta-<key> to the window that currently contains
#       the item target (or the current window if target is not given)
# }}}
# {{{ /awl sort_windows [-yes] XXX NOT YET IMPLEMENTED
#     * sorts your windows as specified in awl_sort_windows setting. Mostly
#       useful only if you don't load awl on startup. -yes overwrites virtual.
# }}}
# }}}
###
# {{{ WISHES
...
sub ir_fe { # try to fix format stuff
	...
	#$x =~ s/(\\|\$)/\\$1/g;
	$x =~ s/(\\)/\\$1/g; # XXX XXX XXX NEEDS INVESTIGATION XXX XXX XXX
...
sub remake () {
	...
	my $height = $screenHeight - #abs
											(Irssi::settings_get_int(set
			'height_adjust'));
...
#~			            . $screenansi->st()
	...
	foreach my $win (@wins) {
		...
		if ($global_hack_alert_tag_header) {
			$display = Irssi::settings_get_str(set 'display_header'); # '|%4%F$C%N';
...
#~							 .	$terminfo->el()
								# XXX BUG? sends el -> will clear next column!
#~							 . $screenansi->st()
	or wlreset();
			}
			$line++;
...
#~	$screenResizing = 1;
#~	# fit screen
#~	system 'screen -x '.$ENV{'STY'}.' -X fit' unless $FIFO_MODE;
	# get size
...
#~		}
		...
		if ($MOUSE_ON) {
			...
			Irssi::command('/^bind -delete meta-[M'); # FIXME evil
...
#~ 		Irssi::print('Copy mode is now ON, '.uc(setc()).' is temporarily '.
#~ 		             'disabled.');
#~ 		if (!$was_disabled) {
#~ 			$screenResizing = 1;
#~ 			screenOff();
...
#~ 		Irssi::print('Copy mode is now OFF, '.uc(setc()).' is enabled.');
#~ 		if ($was_disabled) {
#~ 			$SCREEN_MODE = undef;
#~ 			$screenResizing = 0;
#~ 			wlreset();
